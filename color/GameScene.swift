...
class GameScene: SKScene {
   ...
   let successMessage = [
      "Success!", "Well done!", "Good job!", "Job done!", "Great job!", "Nice!",
      "Bravo!", "You won!", "Nice work!",
   ]

   override init(size: CGSize) {
      backgroundNode = SKSpriteNode(
         color: UIColor(hue: 0.0, saturation: 0.0, brightness: 0.6, alpha: 1),
         size: size
      )
      ...
      )  //SKSpriteNode(imageNamed: "pauseButton")
   ...
   func addBackButton() {
      ...
      pauseButton.position = CGPoint(
         x: pauseButton.size.width * 0.5,
         y: self.size.height - pauseButton.size.height * 0.5 - appDelegate.safeAreaInsets.top
      )
   ...
   func showStarOutline() {
      ...
      starOutline?.position = CGPoint(
         x: 10 + self.size.width - 130,
         y: 10 + self.size.height - 31 - appDelegate.safeAreaInsets.top
      )
      ...
      let action = SKAction.group([
         ...
         SKAction.move(by: CGVector(dx: -10, dy: -10), duration: duration),
      ])
      starOutline?.run(
         SKAction.sequence([SKAction.wait(forDuration: 0.5), action])
      )
   }
   ...
   func hideStarOutline() {
      if starOutline != nil {
         ...
         let action = SKAction.group([
            ...
            SKAction.scale(to: 1.6, duration: duration),
            SKAction.move(by: CGVector(dx: 10, dy: 10), duration: duration),
         ])
         starOutline?.run(
            SKAction.sequence([
               SKAction.wait(forDuration: 0.5), action,
   ...
   func showLevelNumber() {
      ...
      label1.position = CGPoint(
         x: 10 + self.size.width - 30 - space2,
         y: 10 + self.size.height - 48 - appDelegate.safeAreaInsets.top
      )
      addChild(label1)

      label2.position = CGPoint(
         x: 10 + self.size.width - 28 - space2,
         y: 10 + self.size.height - 48 - appDelegate.safeAreaInsets.top
      )
      addChild(label2)
   ...
   func getBluredScreenshot(_ radius: CGFloat) -> UIImage {
      ...
      let finalImage = image?.applyBlurWithRadius(
         radius,
         tintColor: UIColor(white: 0.5, alpha: 0.2),
   ...
   func addLevelShapeToNode(_ node: SKSpriteNode, id: Int, points: [Float]) {
      ...
      path.move(
         to: CGPoint(
            x: menuScaleX * CGFloat(points[0]),
            y: menuScaleX * CGFloat(640 - points[1])
         )
      )  //CGPathMoveToPoint(path, nil,menuScaleX * CGFloat(points[0]),menuScaleX * CGFloat(640-points[1]))
      var idx: Int = 2
      while idx < points.count {
         path.addCurve(
            to: CGPoint(
               x: menuScaleX * CGFloat(points[idx + 4]),
               y: menuScaleX * CGFloat(640 - points[idx + 5])
            ),
            control1: CGPoint(
               x: menuScaleX * CGFloat(points[idx + 0]),
               y: menuScaleX * CGFloat(640 - points[idx + 1])
            ),
            control2: CGPoint(
               x: menuScaleX * CGFloat(points[idx + 2]),
               y: menuScaleX * CGFloat(640 - points[idx + 3])
            )
         )
         idx += 6
      }
   ...
   func showLevel() {
      container.position = CGPoint(
         ...
         y: (self.size.height + self.size.height / 5) / 2.0
      )
   ...
   func restartLevel() {
      if menuShowing {
         ...
         sprite.run(
            SKAction.sequence([
               SKAction.moveTo(
                  x: container.position.x + sprite.size.width,
                  duration: 0.2
               ), SKAction.removeFromParent(),
            ])
         )
   ...
   func showMenu(_ success: Bool) {
      ...
      let shadow = SKSpriteNode(
         color: UIColor(white: 0.0, alpha: 0.2),
         size: CGSize(width: self.size.width, height: 8)
      )
      shadow.anchorPoint = CGPoint(x: 0.5, y: 1)
      shadow.position = CGPoint(x: 0, y: -height)

      let sprite = SKSpriteNode(
         color: UIColor(white: 0.3, alpha: 0.8),
         size: CGSize(width: self.size.width, height: height)
      )
      sprite.anchorPoint = CGPoint(x: 0.5, y: 1)
      sprite.name = "menu"
      sprite.position = CGPoint(
         x: container.position.x - sprite.size.width,
         y: container.position.y + 160
      )
      addChild(sprite)
      ...
      if success {
         ...
         if colorUndone == false {
            ...
            var stars: [Int] =
               (UserDefaults.standard.object(forKey: name) ?? []) as! [Int]
      ...
      sprite.run(
         SKAction.sequence([
            ...
            SKAction.moveTo(x: container.position.x, duration: 0.2),
         ])
      )
   }
   ...
   override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
      if activeTouch == nil {
         ...
         if activeNode == nil {
            ...
            if let node = touchedButton(
               activeTouch!,
               buttons: (menuShowing
                  ? [
                     menuButton, continueButton, restartButton, nextLevelButton,
                     mainMenuButton,
                  ] : [pauseButton])
            ) {
   ...
   override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
      if let touch = activeTouch {
         ...
         if let node = activeNode {
            ...
            let newActiveNode = self.touchedButton(
               touch,
               buttons: (menuShowing
                  ? [
                     menuButton, continueButton, restartButton, nextLevelButton,
   ...
   override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
      if let touch = activeTouch {
         ...
         if let activeNodeUnwrapped = activeNode {
            if [blueButton, greenButton, purpleButton, orangeButton].contains(
               activeNodeUnwrapped
            ) {
   ...
   func checkLevelFinished() {
      ...
      DispatchQueue.main.asyncAfter(
         deadline: DispatchTime.now() + Double((Int64)(1 * NSEC_PER_SEC))
            / Double(NSEC_PER_SEC)
      ) {
         self.showMenu(true)
      }
