...
class MenuScene: SKScene {
   ...
   override func didMove(to view: SKView) {
      ...
      backButton.position = CGPoint(
         x: backButton.size.width * 0.5,
         y: self.size.height - backButton.size.height * 0.5 - appDelegate.safeAreaInsets.top
      )
      ...
      labelmode.position = CGPoint(
         x: size.width / 2,
         y: 4.4 * size.height / 5 - appDelegate.safeAreaInsets.top
      )
   ...
   func spriteFromShapes(_ level: Int) -> SKSpriteNode {
      ...
      if level > unlockedLevel {
         ...
         path.addLine(
            to: CGPoint(x: mv + menuScaleX * 640, y: mv + menuScaleX * 0)
         )  //CGPathAddLineToPoint(path, nil, mv + menuScaleX * 640, mv + menuScaleX * 0)
         path.addLine(
            to: CGPoint(x: mv + menuScaleX * 640, y: mv + menuScaleX * 640)
         )  //CGPathAddLineToPoint(path, nil, mv + menuScaleX * 640, mv + menuScaleX * 640)
         path.addLine(
            to: CGPoint(x: mv + menuScaleX * 0, y: mv + menuScaleX * 640)
         )  //CGPathAddLineToPoint(path, nil, mv + menuScaleX * 0, mv + menuScaleX * 640)
         path.addLine(
            to: CGPoint(x: mv + menuScaleX * 0, y: mv + menuScaleX * 0)
         )  //CGPathAddLineToPoint(path, nil, mv + menuScaleX * 0, mv + menuScaleX * 0)
         path.closeSubpath()
            ...
            if let shapes = levels[level]["shapes"] as? [[String: AnyObject]] {
               for shape in shapes {
                  if let points = shape["points"] as? [Float] {
                     ...
                     path.move(
                        to: CGPoint(
                           x: mv + menuScaleX * CGFloat(points[0]),
                     ...
                     while idx < points.count {
                        path.addCurve(
                           to: CGPoint(
                              x: mv + menuScaleX * CGFloat(points[idx + 4]),
                              y: mv + menuScaleX * CGFloat(points[idx + 5])
                           ),
                           control1: CGPoint(
                              x: mv + menuScaleX * CGFloat(points[idx + 0]),
                              y: mv + menuScaleX * CGFloat(points[idx + 1])
                           ),
                           control2: CGPoint(
                              x: mv + menuScaleX * CGFloat(points[idx + 2]),
                              y: mv + menuScaleX * CGFloat(points[idx + 3])
                           )
                        )
                        idx += 6
                     }
   ...
   func createLevels() {
      ...
      leftLimit =
         self.size.width / 2 - CGFloat(levelsAllowed) * self.levelWidth + self
         .levelWidth / 3
   ...
   override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
      ...
      for touch in (touches) {
         ...
         if dist > 0 {
            ...
            panVelocity =
               (newPos.x - oldPos.x) / CGFloat(timestamp - lastTimestamp)
