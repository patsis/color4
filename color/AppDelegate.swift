...
class AppDelegate: UIResponder, UIApplicationDelegate {
   ...
   var clickSoundAction: SKAction {
      ...
      case 0:
         return SKAction.playSoundFileNamed(
            "select2.mp3",
   ...
   func setupAudioPlayerWithFile(_ file: NSString, type: NSString)
      -> AVAudioPlayer?
   {
      let path = Bundle.main.path(
         forResource: file as String,
   ...
   func pauseAudio() {
      if let audioPlayer = activeAudioPlayer {
         ...
         let delay =
            DispatchTime.now() + Double(Int64(1.0 * Float(NSEC_PER_SEC)))
            / Double(NSEC_PER_SEC)
