...
class MainMenuScene: SKScene {
   ...
   func checkAndShowRatingAlert() {
      if let lastAlertDate = UserDefaults.standard.object(
         forKey: "lastAlertDate"
      ) as? Date {
         if UserDefaults.standard.bool(forKey: "rateAlertOn") {
            let secondsFromLastAlert = Date().timeIntervalSince(lastAlertDate)
            let secondsGameSession = Date().timeIntervalSince(
               appDelegate.gameStartDate as Date
            )
   ...
   func addLogos() {
      ...
      logoNode1.position = CGPoint(
         x: self.size.width / 2,
         y: 0.85 * self.size.height
      )
      logoNode1.zPosition = 1
      addChild(logoNode1)

      let logos = [
         SKSpriteNode(imageNamed: "logo1"), SKSpriteNode(imageNamed: "logo2"),
         SKSpriteNode(imageNamed: "logo3"), SKSpriteNode(imageNamed: "logo4"),
      ]
      for (index, logo) in logos.enumerated() {
         if appDelegate.appDidShowJustNow {
            ...
            logo.position = CGPoint(
               x: self.size.width - 20 - 0.7 * w,
               y: -40 + 0.8 * h + appDelegate.safeAreaInsets.bottom
            )
            logo.setScale(2)
            logo.zPosition = 1
            addChild(logo)
            ...
            let w = logo.size.width
            let h = logo.size.height
            logo.position = CGPoint(x: self.size.width - 0.7 * w, y: 0.8 * h + appDelegate.safeAreaInsets.bottom)
            logo.zPosition = 1
            addChild(logo)
         }
      }
   ...
   func addButtons() {
      ...
      normal1Button.position = CGPoint(
         x: -normal1Button.size.width,
         y: self.size.height / 2 + buttonHeight * 1.2
      )
      normal1Button.zPosition = 1
      addChild(normal1Button)

      normal2Button.position = CGPoint(
         x: -normal2Button.size.width,
         y: self.size.height / 2
      )
      normal2Button.zPosition = 1
      addChild(normal2Button)

      frenzyButton.position = CGPoint(
         x: -frenzyButton.size.width,
         y: self.size.height / 2 - buttonHeight * 1.2
      )
      ...
      let stars: [Int] =
         (UserDefaults.standard.object(forKey: "normalstars") ?? []) as! [Int]
      let starsNeeded = max(20 - stars.count, 0)
      if starsNeeded > 0 {
         let locked = SKSpriteNode(imageNamed: "frenzyLocked")
         locked.name = "frenzylock"
         locked.position = CGPoint(
            x: 2,
            y: 11 + (locked.size.height - frenzyButton.size.height) / 2
         )
         ...
         labelLock.position = CGPoint(
            x: -55,
            y: 10 + (locked.size.height - frenzyButton.size.height) / 2
         )
         frenzyButton.addChild(labelLock)
         labelLock.name = "frenzylabel"
         frenzyLocked = true
      } else {
         let frenzyUnlocked = UserDefaults.standard.bool(
            forKey: "frenzyunlocked"
         )
         if !frenzyUnlocked {
            let locked = SKSpriteNode(imageNamed: "frenzyLocked")
            locked.name = "frenzylock"
            ...
            star.position = CGPoint(
               x: -97,
               y: 12 + (locked.size.height - frenzyButton.size.height) / 2
            )
            star.setScale(0.3)
            star.alpha = 0.7
            star.run(
      ...
      let actionNormal1 = SKAction.moveTo(
         x: space + normal1Button.size.width / 2,
         duration: 0.6
      )
      ...
      let actionFrenzy = SKAction.moveTo(
         x: self.size.width - frenzyButton.size.width / 2 - space,
         duration: 1
      )
   ...
   func addBottomButtons() {
      ...
      pos += 1
      addBottomButton(
         appDelegate.musicDisabled ? "musicOff" : "musicOn",
         delay: delay + wait * Double(pos),
         pos: pos
      )
      pos += 1
   }
   ...
   func addBottomButton(_ name: String, delay: TimeInterval, pos: CGFloat) {
      ...
      button.position = CGPoint(x: 60 + (pos - 1) * 100, y: 60 + appDelegate.safeAreaInsets.bottom)
      button.setScale(0.2)
      button.alpha = 0
      button.name = name
      button.zPosition = 1
      ...
      button.run(
         SKAction.sequence([
            wait,
            SKAction.group([alpha, SKAction.sequence([scale1, scale2])]),
         ])
      )
   ...
   func addRects() {
      for _ in 0..<16 {
         ...
         let color = UIColor(
            white: 1,
            alpha: 0.1 + CGFloat(arc4random_uniform(20)) / 100.0
         )
         ...
         sprite.position = CGPoint(
            x: cos(r) * 100 + self.size.width / 2,
            y: sin(r) * 100 + self.size.height / 2
         )
         rects.append(
            (sprite: sprite, size: size, velocity: CGVector(dx: 0, dy: 0))
         )
         sprite.alpha = 0
         sprite.zPosition = 0
   ...
   func touchedButton(_ touch: UITouch, buttons: [SKSpriteNode?])
      -> SKSpriteNode?
   {
      var node: SKSpriteNode?
      var maxz: CGFloat = -1
   ...
   override func update(_ currentTime: TimeInterval) {
      ...
      for (i, irect): (Int, RectTuple) in rects.enumerated() {
         ...
         let iv = CGVector(
            dx: cos(angle) * dist / (factorC),
            dy: sin(angle) * dist / (factorC)
         )
         rects[i].velocity = CGVector(
            dx: irect.velocity.dx + iv.dx + shakeSpeedX * dx / 100,
            dy: irect.velocity.dy + iv.dy * 0.6 + shakeSpeedY * dy / 200
         )
      }
      ...
      for (i, irect): (Int, RectTuple) in rects.enumerated() {
         for (j, jrect): (Int, RectTuple) in rects.enumerated() {
            if i != j {
               ...
               let dist = sqrt(dx * dx + dy * dy)
               let iv = CGVector(
                  dx: CGFloat(jrect.size) * cos(angle) / (factor * dist),
                  dy: CGFloat(jrect.size) * sin(angle) / (factor * dist)
               )
               let jv = CGVector(
                  dx: CGFloat(irect.size) * cos(angle) / (factor * dist),
                  dy: CGFloat(irect.size) * sin(angle) / (factor * dist)
               )
               rects[i].velocity = CGVector(
                  dx: irect.velocity.dx + iv.dx,
                  dy: irect.velocity.dy + iv.dy
               )
               rects[j].velocity = CGVector(
                  dx: jrect.velocity.dx - jv.dx,
                  dy: jrect.velocity.dy - jv.dy
               )
            }
         }
      }
      ...
      for (_, irect): (Int, RectTuple) in rects.enumerated() {
         irect.sprite.position = CGPoint(
            x: irect.sprite.position.x + irect.velocity.dx,
            y: irect.sprite.position.y + irect.velocity.dy
         )
      }

      for (i, irect): (Int, RectTuple) in rects.enumerated() {
         rects[i].velocity = CGVector(
            dx: irect.velocity.dx * 0.33,
            dy: irect.velocity.dy * 0.33
         )
      }
   }
